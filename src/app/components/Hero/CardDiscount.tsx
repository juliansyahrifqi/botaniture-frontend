...
export default function CardDiscount(props: CardDiscountProps) {
  const { background, promoName, data} = props;
  ...
  return (
    <div className={`relative card p-5 rounded-3xl grid grid-cols-3 items-center gap-4 ${background === 'primary' ? 'bg-primary bg-[url(/bg-right.png)]' : 'bg-hero-secondary bg-[url(/bg-right-2.png)]'} bg-no-repeat bg-contain bg-right`}>
      <Link href={`/product/${data.product_slug}`} className="absolute bg-white rounded-full p-2 right-3 top-3 bg-opacity-40 hover:bg-black hover:ring-2 hover:ring-white cursor-pointer transition-all ease-in-out duration-300">
      ...
      <Image src={`${process.env.IMAGE_URL}/product/${props.data.product_image}`} alt={props.data.product_name} width={200} height={250} />
    </div>
  )
}
